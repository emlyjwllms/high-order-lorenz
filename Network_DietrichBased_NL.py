...
class ModelBuilder:
    ...
    def define_normal_distribution(#xn,
        ...
        #Get individual coordinates of bar_xn
        if len(bar_xn.shape) == 1:
            xbar = tf.reshape(bar_xn[0], [1])
            ybar = tf.reshape(bar_xn[1], [1])
            zbar = tf.reshape(bar_xn[2], [1])
        else:
            xbar, ybar, zbar = bar_xn[:,0], bar_xn[:,1], bar_xn[:,2]
        
        # Compute xdot entries evaluated at bar_xn
        f1 = sigma*(ybar - xbar)
        f2 = xbar*(r - zbar) - ybar
        f3 = xbar*ybar - beta*zbar
        ...
        if diffusivity_type == "diagonal":
            ...
            approx_normal = ModelBuilder.define_normal_distribution_triangular(bar_xn, step_size_, drift_, diff_matrix)
        elif diffusivity_type == "spd":
            # form the normal distribution with SPD matrix
            approx_normal = ModelBuilder.define_normal_distribution_spd(bar_xn, step_size_, drift_, diff_matrix)
        else:
            raise ValueError(
...
class SDEIdentification:
    ...
    def eval_sigma(self, bar_xn):
        ...
        sigma_theta = self.model.call_xn(bar_xn)
        return K.eval(sigma_theta)
    
    def sample_bar_xn1(self,
                         #xn,
                         bar_xn,
                         step_size,
                         jac_par,
                         diffusivity_type):
        """
        Samples a new state (bar_xn1) using the Euler-Maruyama scheme.
        ...
        -------
        array-like
            The sampled next state filtered (bar_xn1).
        """
        sigma_theta = self.model.call_xn(bar_xn)
        ...
        bar_xn1 = approx_normal.sample()
        return keras.backend.eval(bar_xn1)
    ...
    **kwargs
    ...
    def euler_maruyama_pdf(bar_xn, bar_xn1, step_size, jac_par, model_, diffusivity_type="diagonal"):
        ...
        approx_normal = ModelBuilder.define_normal_distribution(#xn,
                                                                ...
                                                                sigma_theta,
                                                                diffusivity_type)
        return approx_normal.log_prob(bar_xn1)
