...
class ModelBuilder:
    ...
    @staticmethod
    def define_normal_distribution(step_size_,
                                   sigma_theta, 
                                   diffusivity_type):
        """
        ...
        n = tf.shape(sigma_theta)[0]

        #Compute drift term
        drift_ = tf.zeros([n,3],dtype=tf.dtypes.float64)
        ...
        if diffusivity_type == "diagonal":
            ...
            approx_normal = ModelBuilder.define_normal_distribution_triangular(step_size_, drift_, diff_matrix)
        elif diffusivity_type == "spd":
            # form the normal distribution with SPD matrix
            approx_normal = ModelBuilder.define_normal_distribution_spd(step_size_, drift_, diff_matrix)
    ...
    @staticmethod
    def define_normal_distribution_triangular(step_size_, drift_, diffusivity_tril_):
        """
    ...
    @staticmethod
    def define_normal_distribution_spd(step_size_, drift_, diffusivity_spd_):
        """
        ...
        # square the matrix so that the cholesky decomposition does not change the eigenvalues
...
class SDEIdentification:
    ...
    def train_model(self, bar_xn, bar_xn1, step_size, validation_split=0.1, n_epochs=100, batch_size=1000, callbacks=[]):
        """
        ...
        """
        print(f"training nonlinear model for {n_epochs} epochs with {int(bar_xn.shape[0] * (1 - validation_split))} data points"
              f", validating with {int(bar_xn.shape[0] * validation_split)}")

        #features = np.column_stack([xn, bar_xn])
        features = bar_xn

        full_dataset = np.column_stack([features, bar_xn1])
    ...
    def eval_sigma(self, bar_xn1):
        ...
        sigma_theta = self.model.call_xn(bar_xn1)
        return K.eval(sigma_theta)
    
    def sample_sigma(self, bar_xn1):
        """
        Samples a new state (s) using the Euler-Maruyama scheme.
        ...
        -------
        array-like
            The sampled next state filtered (s).
        """
        sigma_theta = self.model.call_xn(bar_xn1)

        # approx_normal = ModelBuilder.define_normal_distribution(step_size,
        #                                                         sigma_theta,
        #                                                         diffusivity_type)

        # s = approx_normal.sample()
        return keras.backend.eval(sigma_theta)
...
class SDEApproximationNetwork(tf.keras.Model):
    ...
    for a nonlinear Lorenz system. It is trained using an Euler-Maruyama scheme-based loss function.
    ...
    **kwargs
    ...
    @staticmethod
    def euler_maruyama_pdf(bar_xn, bar_xn1, step_size, jac_par, model_, diffusivity_type="diagonal"):
        """
        ...
        #sigma_theta = model_(tf.concat([xn, bar_xn], axis=1)) #Call to the model defined in ModelBuilder.diff_network
        sigma_theta = model_(bar_xn1)

        approx_normal = ModelBuilder.define_normal_distribution(step_size,
                                                                sigma_theta,
                                                                diffusivity_type)
        
        #Define constant tensors for using in dynamics computation
        ...
        #Get individual coordinates of bar_xn1
        if len(bar_xn1.shape) == 1:
            xbar1 = tf.reshape(bar_xn1[0], [1])
            ybar1 = tf.reshape(bar_xn1[1], [1])
            zbar1 = tf.reshape(bar_xn1[2], [1])
        else:
            xbar1, ybar1, zbar1 = bar_xn1[:,0], bar_xn1[:,1], bar_xn1[:,2]
        
        # Compute xdot entries evaluated at bar_xn1
        f1 = sigma*(ybar1 - xbar1)
        f2 = xbar1*(r - zbar1) - ybar1
        f3 = xbar1*ybar1 - beta*zbar1
                
        # Creates dynamics tensor
        f = tf.stack([f1,f2,f3],axis=1)

        return approx_normal.log_prob(bar_xn1 - bar_xn - f * step_size)
    ...
    def split_inputs(inputs, step_size=None):
        ...
        -------
        tuple
            A tuple of tensors (xn, bar_xn, bar_xn1, step_size).
        """
        ...
        if step_size is not None:
            ...
            #n_each = (n_total - 1) // 3
            n_each = (n_total - 1) // 2
            #x_n, bar_xn, bar_xn1, step_size = tf.split(inputs, num_or_size_splits=[n_each, n_each, n_each, 1], axis=1)
            bar_xn, bar_xn1, step_size = tf.split(inputs, num_or_size_splits=[n_each, n_each, 1], axis=1)
        else:
            #n_each = n_total // 3
            n_each = n_total // 2
            #x_n, bar_xn, bar_xn1 = tf.split(inputs, num_or_size_splits=[n_each, n_each, n_each], axis=1)
            bar_xn, bar_xn1 = tf.split(inputs, num_or_size_splits=[n_each, n_each], axis=1)

        return bar_xn, bar_xn1, step_size

    def call_xn(self, bar_xn1):
        """
        ...
        xn : tensor
            The current state.
        bar_xn : tensor
            The filtered to the current state.
        ...
        """
        #assert(len(xn.shape) == len(bar_xn.shape)), "Shape dimension mismatch between xn and bar_xn"
        if len(bar_xn1.shape) == 1:
            #xn = xn.reshape((1,3))
            bar_xn1 = bar_xn1.reshape((1,3))
            #arguments = tf.concat([xn, bar_xn], axis=1)
            arguments = bar_xn1
    ...
    def call(self, inputs):
        ...
        ----------
        inputs : tensor
            The input tensor containing all of (xn, bar_xn, bar_xn1, step_size).
        ...
        """
        bar_xn, bar_xn1, step_size = SDEApproximationNetwork.split_inputs(inputs, self.step_size)

        if self.method == "euler":
            log_prob = SDEApproximationNetwork.euler_maruyama_pdf(bar_xn, bar_xn1, step_size, self.jac_par, self.sde_model,
                                                                  diffusivity_type=self.diffusivity_type)
        ...
        loss = loss + 2 * np.log(2 * np.pi) / np.log(10) * bar_xn.shape[1]

        self.add_loss(loss)
        self.add_metric(distortion, name="distortion", aggregation="mean")

        #return self.sde_model(tf.concat([xn, bar_xn], axis=1))
        return self.sde_model(bar_xn1)
