...
from Network_DietrichBased_NL import (
                                    SDEIdentification,
                                    ModelBuilder,
...
if train:
    ...
    plt.ylabel("Loss")
    plt.legend()
    plt.savefig(file_path + 'plots/training-NL-h' + str(int(1/h)) + '.png')
    plt.show()
...
for n in range(Nh-1):
    ...
    # NN method
    xbar_NN[n+1,:] = sde_i.sample_bar_xn1(xbar_NN[n,:], h, jac_par, diffusivity_type)

    # x_NN[n+1,:] = xbar_NN[n+1,:] + xp[n+1,:]

    # forward Euler method
    # x_FE[n+1,:] = x_FE[n,:] + f(x_FE[n,:],jac_par)*dt

print("saving data")
np.savez('data/diffusion-path-NL-h' + str(int(1/h)) + '.npz', t=t, xbar_NN=xbar_NN )